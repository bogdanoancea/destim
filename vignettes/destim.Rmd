...
The function sstates returns the smooth states as a matrix of *number of states* $\times$ *number of observations* (missing values included) dimensions. So each column represents the spatial distribution in its corresponding time slot.
```{r}
dim(sstates(model, obs))
image(matrix(sstates(model, obs)[,4], ncol = 10))
```

The function scpstates returns the (smooth) joint mass probability 
